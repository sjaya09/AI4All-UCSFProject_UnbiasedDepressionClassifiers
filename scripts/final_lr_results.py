...
if __name__ == '__main__':
    ...
    lr_test_score = roc_auc_score(y_test.values, lr_pipe.predict_proba(X_test)[:, 1])
    final_lr_results_df = pd.DataFrame(data=['lr_training_score','lr_validation_score','lr_test_score'], columns=['scores'])
    final_lr_results_df.to_csv('notebooks/final_lr_results.csv', index=False)
