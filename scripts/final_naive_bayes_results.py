...
if __name__ == '__main__':
    ...
    nb_test_score = roc_auc_score(y_test.values, nb_pipe.predict_proba(X_test)[:, 1])
    final_nb_results_df = pd.DataFrame(data=['nb_training_score','nb_validation_score','nb_test_score'], columns=['scores'])
    final_nb_results_df.to_csv('notebooks/final_nb_results.csv', index=False)
