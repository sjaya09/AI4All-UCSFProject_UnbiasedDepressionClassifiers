...
if __name__ == '__main__':
    ...
    lda_test_score = roc_auc_score(y_test.values, lda_pipe.predict_proba(X_test)[:, 1])
    final_lda_results_df = pd.DataFrame(data=['lda_training_score','lda_validation_score','lda_test_score'], columns=['scores'])
    final_lda_results_df.to_csv('notebooks/final_lda_results.csv', index=False)
